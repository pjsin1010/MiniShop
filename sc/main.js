...
function pop10()//판매글
{
	window.open("itembuywrite.html","판매등록","width=900, height=500, left=550, top=240");	
}
function pop20()//구매글
{

	window.open("itemwrite.html","회원가입","width=900, height=500, left=550, top=240");
}
...
function firstChange() {
 var x = document.form.dea.options.selectedIndex;
 var groups=document.form.dea.options.length;
...
function firstChange1() {
 var x = document.form.si.options.selectedIndex;
 var groups=document.form.si.options.length;
 var group=new Array(groups);
 for (i=0; i<groups; i++)
  group[i]=new Array();
